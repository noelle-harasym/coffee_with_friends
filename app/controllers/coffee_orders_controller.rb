class CoffeeOrdersController < ApplicationController
  ...
  def index
    ...
    @coffee_orders = @q.result(:distinct => true).includes(:user, :coffee_type, :dairy_type, :sweetness_type).page(params[:page]).per(10)
  end
  ...
  private
    ...
    def coffee_order_params
      params.require(:coffee_order).permit(:user_id, :coffee_type_id, :modification_iced, :dairy_type_id, :sweetness_type_id, :modification_other, :rating, :picture, :date)
    end
end
